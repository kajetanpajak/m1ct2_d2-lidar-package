...
int node_start(int argc, char **argv)
{
	...
	node->declare_parameter<std::string>("port", "/dev/serial/by-id/usb-1a86_USB_Serial-if00-port0");
	...
	while(rclcpp::ok())
	{
		...
		if(data_handling(scan))
		{
			...
			for(long unsigned int i=0; i < scan.points.size(); i++) {
